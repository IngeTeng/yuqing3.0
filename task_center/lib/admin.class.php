...
class Admin {
	...
	static public function add($account, $password, $group){
		...
		//检查参数
		if(empty($account))throw new MyException('账号不能为空', 101);
		if(empty($group))throw new MyException('管理员组不能为空', 102);
        ...
        $rs = Table_admin::add($account, $password, $group);
		if($rs > 0){
			//记录管理员日志log表
			$msg = '成功添加管理员('.$account.')';
			Adminlog::add($msg);
	...
	static public function edit($id, $account, $group){
        ...
        $rs = Table_admin::edit($id, $account, $group);
        if($rs >= 0){
            $msg = '修改管理员('.$id.')信息成功!';
            Adminlog::add($msg);
    ...
    //一个页面多种权限的情况
    static function checkAuth_page($powerId, $auth){
        if(empty($powerId))throw new MyException('权限ID不能为空', 101);
        //if(empty($auth))throw new MyException('权限序列不能为空', 102);
